...
const PROTOCOL_OPTIONS = [
  { id: 'openai-compatible', label: 'Chat Completions' },
...
function Channels({ state, discovery, csrf, setNotice, onRefresh, openDialog }: { state: Json | null; discovery: Json | null; csrf: string; setNotice: (value: { kind: 'ok' | 'error' | 'info'; text: string } | null) => void; onRefresh: () => Promise<void>; openDialog: (value: { title: string; body: string; action: () => Promise<void> } | null) => void }) {
  ...
  const syncableChannels = channels.filter((channel: Json) => channel.enabled || channel.staged)
  ...
  const setMode = (channel: Json, mode: 'production' | 'staged' | 'disabled') => {
    if (mode === 'production' && Number(channel.modelCount ?? 0) === 0) {
      setNotice({ kind: 'error', text: '请先同步该渠道的模型目录，再启用生产。' })
      return
    }
    const body = channel.busy
      ? '该渠道当前有在途请求，会先进入排空状态。'
      : '配置写入后需要应用或重启才能生效。非对话模型不执行固定诗词测活，请通过真实调用记录观察可用性。'
    openDialog({ title: `${mode === 'production' ? '启用生产' : mode === 'staged' ? '设为待测试' : '停用'} ${channel.id}`, body, action: () => mutate(`/admin/api/channels/${encodeURIComponent(channel.id)}`, { method: 'PATCH', body: JSON.stringify(mode === 'production' ? { enabled: true, staged: false } : mode === 'staged' ? { enabled: false, staged: true } : { enabled: false, staged: false }) }, '渠道状态已更新') })
  }
  ...
  const sync = async () => {
    if (syncing) return
    setSyncing(true)
    setNotice({ kind: 'info', text: `正在读取 ${syncChannel || '生产渠道'} 的 /models…` })
    try {
      const result = await api('/admin/api/model-sync', { method: 'POST', csrf, body: JSON.stringify({ channels: syncChannel ? [syncChannel] : [] }) })
      setNotice({ kind: 'ok', text: `模型目录同步完成 · ${result.channels?.reduce((sum: number, item: Json) => sum + Number(item.discovered ?? 0), 0) ?? 0} 个模型` })
      await onRefresh()
    } catch (error) {
      setNotice({ kind: 'error', text: `模型目录同步失败：${errorMessage(error)}` })
    } finally {
      setSyncing(false)
    }
  }
  ...
  return <div className="view-stack"><section className="band"><div className="section-heading"><div><div className="eyebrow">PHYSICAL CHANNELS</div><h2>渠道目录</h2></div><span className="muted">{channels.length} 个渠道</span></div><div className="table-scroll"><table><thead><tr><th>渠道</th><th>Base URL</th><th>生命周期</th><th>健康</th><th>模型</th><th>操作</th></tr></thead><tbody>{channels.map((channel: Json) => <tr key={channel.id}><td><strong>{channel.name}</strong><small>{channel.id}</small></td><td><code>{channel.baseUrl}</code></td><td><StateTag channel={channel} /></td><td><HealthTag value={channel.health} lastStatusCode={channel.lastStatusCode} /></td><td>{channel.modelCount ?? 0}</td><td><div className="inline-actions"><button className="icon-button bordered" title="编辑渠道" onClick={() => beginEdit(channel)}><Settings2 size={15} /></button>{channel.staged ? <><button className="button tiny" onClick={() => setMode(channel, 'production')}><Check size={14} />设为生产</button><button className="button tiny subtle" onClick={() => setMode(channel, 'disabled')}><X size={14} />移出待测</button></> : channel.enabled ? <button className="button tiny subtle" onClick={() => setMode(channel, 'disabled')}><X size={14} />停用</button> : <><button className="button tiny" onClick={() => setMode(channel, 'staged')}><Activity size={14} />待测试</button><button className="button tiny subtle" onClick={() => setMode(channel, 'production')}><Check size={14} />启用</button></>} {channel.health === 'auth-failed' && <button className="button tiny subtle" onClick={() => openDialog({ title: `重新尝试 ${channel.id}`, body: '只清除网关本地的认证故障隔离，不会发送上游请求；清除后请从业务端重新调用。', action: () => mutate(`/admin/api/channels/${encodeURIComponent(channel.id)}/retry`, { method: 'POST' }, '已解除本地认证隔离') })}><RotateCcw size={14} />重新尝试</button>} {!channel.enabled && !channel.staged && !channel.busy && <button className="icon-button danger" title="删除渠道" onClick={() => openDialog({ title: `删除 ${channel.id}`, body: '删除前必须确认该渠道没有别名或逻辑组引用。', action: () => mutate(`/admin/api/channels/${encodeURIComponent(channel.id)}`, { method: 'DELETE' }, '渠道已删除') })}><Trash2 size={15} /></button>}</div></td></tr>)}</tbody></table></div></section>{editingChannel && <section className="band"><div className="section-heading"><div><div className="eyebrow">CHANNEL SETTINGS</div><h2>编辑 {editingChannel.id}</h2></div><button className="icon-button" title="取消编辑" onClick={cancelEdit}><X size={17} /></button></div><form ref={editFormRef} className="form-grid" onSubmit={saveEdit}><label>名称<input required maxLength={80} value={editForm.name} onChange={event => setEditForm({ ...editForm, name: event.target.value })} /></label><label>Base URL<input required type="url" value={editForm.baseUrl} onChange={event => setEditForm({ ...editForm, baseUrl: event.target.value })} /></label><label>替换 API key<input type="password" minLength={8} autoComplete="new-password" placeholder={editingChannel.hasApiKey ? '留空保持现有密钥' : '输入新的 API key'} value={editForm.apiKey} onChange={event => setEditForm({ ...editForm, apiKey: event.target.value })} /></label><label>默认协议<select value={editForm.protocol} onChange={event => setEditForm({ ...editForm, protocol: event.target.value })}><option value="responses">OpenAI Responses</option><option value="openai-compatible">OpenAI Chat Completions</option><option value="claude">Anthropic Messages</option></select></label><label>优先级<input required step="1" type="number" value={editForm.priority} onChange={event => setEditForm({ ...editForm, priority: event.target.value })} /></label><div className="form-actions"><button className="button primary" type="submit"><Save size={16} />保存修改</button><button className="button subtle" type="button" onClick={cancelEdit}>取消</button></div></form><p className="muted compact">现有 API key 不会读取或回显；留空即可保持不变。默认协议仅影响未单独设置模型协议的模型；同一渠道内的模型可以使用不同协议。修改会创建私有 revision，并在应用或重启后生效。</p></section>}<ManualModelForm channels={channels} csrf={csrf} setNotice={setNotice} onRefresh={onRefresh} /> <section className="two-column"><div className="band"><div className="section-heading"><div><div className="eyebrow">MODEL DIRECTORY</div><h2>同步模型</h2></div><Database size={18} /></div><div className="form-row"><select value={syncChannel} onChange={event => setSyncChannel(event.target.value)}><option value="">选择渠道（留空同步全部生产）</option>{syncableChannels.map((channel: Json) => <option key={channel.id} value={channel.id}>{channel.name} · {channel.id} · {channel.staged ? '待测试' : '生产'}</option>)}</select><button className="button primary" disabled={!syncableChannels.length || syncing} onClick={() => void sync()}><RefreshCw size={16} className={syncing ? 'spin' : ''} />{syncing ? '读取中…' : '读取 /models'}</button></div><p className="muted compact">同步只读取模型目录，不发送生成请求；待测试渠道也可显式同步。</p></div><div className="band"><div className="section-heading"><div><div className="eyebrow">DISCOVERY</div><h2>渠道发现</h2></div><Search size={18} /></div><Discovery discovery={discovery} csrf={csrf} setNotice={setNotice} onRefresh={onRefresh} /></div></section><section className="band"><div className="section-heading"><div><div className="eyebrow">NEW CHANNEL</div><h2>新增待测试渠道</h2></div><span className="muted">密钥只写入配置；默认协议仅作用于未单独设置协议的模型</span></div><form ref={formRef} className="form-grid" onSubmit={submit}><label>渠道 ID<input required pattern="[a-z][a-z0-9-]{0,31}" value={form.id} onChange={event => setForm({ ...form, id: event.target.value })} /></label><label>名称<input required maxLength={80} value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} /></label><label>Base URL<input required type="url" value={form.baseUrl} onChange={event => setForm({ ...form, baseUrl: event.target.value })} /></label><label>API key<input required minLength={8} type="password" autoComplete="new-password" value={form.apiKey} onChange={event => setForm({ ...form, apiKey: event.target.value })} /></label><label>默认协议<select value={form.protocol} onChange={event => setForm({ ...form, protocol: event.target.value })}><option value="responses">OpenAI Responses</option><option value="openai-compatible">OpenAI Chat Completions</option><option value="claude">Anthropic Messages</option></select></label><label>优先级<input required step="1" type="number" value={form.priority} onChange={event => setForm({ ...form, priority: event.target.value })} /></label><div className="form-actions"><button className="button primary" type="submit"><Save size={16} />新增渠道</button><button className="button subtle" type="button" onClick={() => void submitChannel(true)}><RefreshCw size={16} />新增并同步</button></div></form></section></div>
}

function ManualModelForm({ channels, csrf, setNotice, onRefresh }: { channels: Json[]; csrf: string; setNotice: (value: { kind: 'ok' | 'error' | 'info'; text: string } | null) => void; onRefresh: () => Promise<void> }) {
  const [channel, setChannel] = useState('')
  const [model, setModel] = useState('')
  const [kind, setKind] = useState('embedding')
  const [protocol, setProtocol] = useState('openai-compatible')
  const [saving, setSaving] = useState(false)
  const submit = async (event: FormEvent) => {
    event.preventDefault()
    if (!channel || !model.trim() || saving) return
    setSaving(true)
    try {
      const result = await api('/admin/api/models', { method: 'POST', csrf, body: JSON.stringify({ channel, model: model.trim(), kind, protocol }) })
      setNotice({ kind: 'ok', text: `已登记 ${channel}/${model.trim()} · 等待应用` })
      setModel('')
      await onRefresh()
      return result
    } catch (error) {
      setNotice({ kind: 'error', text: `手动登记失败：${errorMessage(error)}` })
    } finally {
      setSaving(false)
    }
  }
  return <section className="band"><div className="section-heading"><div><div className="eyebrow">MANUAL MODEL</div><h2>手动登记模型</h2></div><span className="muted">上游禁止 /models 时使用</span></div><form className="form-grid" onSubmit={submit}><label>渠道<select required value={channel} onChange={event => setChannel(event.target.value)}><option value="">选择渠道</option>{channels.map(item => <option key={item.id} value={item.id}>{item.name} · {item.id} · {item.staged ? '待测试' : item.enabled ? '生产' : '已停用'}</option>)}</select></label><label>上游模型 ID<input required value={model} onChange={event => setModel(event.target.value)} placeholder="例如 text-embedding-3-small" /></label><label>模型类型<select value={kind} onChange={event => setKind(event.target.value)}><option value="embedding">Embedding</option><option value="rerank">Rerank</option><option value="audio">Audio</option><option value="image">Image</option><option value="video">Video</option><option value="ocr">OCR</option><option value="moderation">Moderation</option><option value="generation">Generation</option></select></label><label>协议<select value={protocol} onChange={event => setProtocol(event.target.value)}><option value="openai-compatible">Chat Completions / OpenAI compatible</option><option value="responses">Responses</option><option value="claude">Anthropic Messages</option></select></label><div className="form-actions"><button className="button primary" type="submit" disabled={saving}>{saving ? '登记中…' : '登记模型'}</button></div></form><p className="muted compact">只写入精确模型目录，不发送上游请求。登记后仍保持当前渠道状态；应用或重启后生效。embedding 只能通过 `/v1/embeddings` 调用。</p></section>
}
...
function Models({ models, state, csrf, setNotice, onRefresh, openDialog }: { models: Json[]; state: Json | null; csrf: string; setNotice: (value: { kind: 'ok' | 'error' | 'info'; text: string } | null) => void; onRefresh: () => Promise<void>; openDialog: (value: { title: string; body: string; action: () => Promise<void> } | null) => void }) {
  ...
  const [testResults, setTestResults] = useState<Record<string, Json>>({})
  const candidate = candidates.find(item => item.directId === selected)
  const restart = state?.restartRequired === true
  const resultKey = `${selected}\0${protocol}`
  const latestResult = testResults[resultKey] ?? candidate?.protocolTests?.[protocol] ?? (candidate?.lastTest?.protocol === protocol ? candidate.lastTest : null)
  const verified = latestResult?.ok === true && Date.parse(latestResult.testedAt) >= Date.now() - 30 * 60 * 1000
  const protocolApplicable = candidate?.kind === 'generation' ? verified : Boolean(candidate)
  ...
  const selectCandidate = (event: ChangeEvent<HTMLSelectElement>) => {
    const directId = event.target.value
    setSelected(directId)
    setProtocol(candidates.find(item => item.directId === directId)?.protocol ?? 'responses')
  }
  const test = async () => {
    if (!selected) return
    if (candidate?.kind !== 'generation' || candidate?.canaryEligible === false) {
      setNotice({ kind: 'info', text: '该模型不是对话生成模型，不适用固定诗词测活；请通过真实调用记录观察可用性。' })
      return
    }
    try {
      const result = await api('/admin/api/tests', { method: 'POST', csrf, body: JSON.stringify({ model: selected, protocolOverride: protocol }) })
      setTestResults(current => ({ ...current, [resultKey]: result }))
      setNotice({
        kind: result.ok ? 'ok' : 'error',
        text: result.ok
          ? `${protocolLabel(protocol)} 测活成功 · ${result.latencyMs} ms · 正文 ${result.contentLength} 字`
          : `${protocolLabel(protocol)} 测活失败 · ${canaryErrorLabel(result.error)}${result.statusCode ? ` · HTTP ${result.statusCode}` : ''}`
      })
      await onRefresh()
    } catch (error) {
      setNotice({ kind: 'error', text: errorMessage(error) })
  ...
  const applyProtocol = () => {
    if (!candidate || !protocolApplicable) return
    const nonGeneration = candidate.kind !== 'generation'
    openDialog({
      title: `应用 ${protocolLabel(protocol)} 到当前模型`,
      body: nonGeneration
        ? `将 ${candidate.directId} 的上游协议元数据设置为 ${protocolLabel(protocol)}。该模型不执行诗词测活，配置只影响这个精确模型；实际可用性由对应业务端点和真实调用记录确认。`
        : `将 ${candidate.directId} 的上游协议设置为 ${protocolLabel(protocol)}。协议应用只影响这个精确模型；同渠道其他模型保持不变。这会创建待应用 revision，不会自动重启。`,
      action: async () => {
        try {
          const result = await api('/admin/api/protocols/apply', {
            method: 'POST',
            csrf,
            body: JSON.stringify({
              scope: 'model',
              ...
              protocol,
              confirmTarget: candidate.directId,
              confirmProtocol: protocol,
              ...(nonGeneration ? { confirmNonGeneration: true } : {})
            })
          })
          setNotice({ kind: 'ok', text: `协议配置已保存，等待应用 · ${result.revision ?? ''}` })
          await onRefresh()
        } catch (error) {
          setNotice({ kind: 'error', text: errorMessage(error) })
        }
  ...
  return <div className="view-stack"><section className="band"><div className="section-heading"><div><div className="eyebrow">MODEL CONTROL</div><h2>精确模型测活与路由</h2></div><span className="muted">生成模型使用固定诗词任务；非对话模型仅目录同步与真实调用观测</span></div><div className="model-toolbar"><select value={selected} onChange={selectCandidate}><option value="">请选择精确模型</option>{candidates.map(item => <option key={item.directId} value={item.directId}>{item.directId} · {item.kind === 'generation' ? '生成' : item.kind}</option>)}</select><button className="button primary" disabled={Boolean(blocked) || candidate?.canaryEligible === false} onClick={() => void test()}><Play size={16} />按此协议测活</button><button className="button" disabled={!candidate || candidate.status === 'disabled' || candidate.kind !== 'generation'} onClick={() => confirmAlias('coding-main')}><ArrowUp size={16} />coding-main</button><button className="button subtle" disabled={!candidate || candidate.status === 'disabled' || candidate.kind !== 'generation'} onClick={() => confirmAlias('coding-backup')}><ArrowDown size={16} />coding-backup</button><button className="button subtle" disabled={!candidate} onClick={toggle}>{candidate?.status === 'disabled' ? <><Check size={16} />恢复</> : <><X size={16} />禁用</>}</button></div><div className="protocol-row"><span>{candidate?.kind === 'generation' ? '试测协议' : '端点协议'}</span><div className="segmented-control" role="group" aria-label="模型协议">{PROTOCOL_OPTIONS.map(item => <button key={item.id} className={protocol === item.id ? 'active' : ''} disabled={!candidate || restart} onClick={() => setProtocol(item.id)}>{item.label}</button>)}</div><button className="button subtle" disabled={!candidate || !protocolApplicable || restart} onClick={() => applyProtocol()}><Save size={16} />应用到当前模型</button></div>{candidate?.kind !== 'generation' && <p className="muted compact">非生成模型不执行诗词测活；协议可按精确模型显式设置，随后通过对应业务端点和真实调用记录确认。</p>}{restart && <p className="inline-warning"><AlertTriangle size={16} />配置待应用，测活暂时禁用。</p>}{candidate && <div className="candidate-inspector"><StateTag channel={candidate} /><span>当前协议：{protocolLabel(candidate.protocol)}</span><span>模式：{streamingLabel(candidate.streaming)}</span><span>调度：{reasonText(candidate.scheduling?.reasonCodes)}</span><span>证据：{evidenceText(candidate.scheduling?.evidence)}</span></div>}{latestResult && <div className={`canary-result ${latestResult.ok ? 'success' : 'danger'}`}><div><strong>{protocolLabel(latestResult.protocol)} · {latestResult.ok ? '通过' : canaryErrorLabel(latestResult.error)}</strong><small>{formatCanaryDiagnostics(latestResult)}</small></div><span className={`state-tag ${latestResult.ok ? 'success' : 'danger'}`}>{latestResult.latencyMs} ms</span></div>}</section><section className="band"><div className="section-heading"><div><div className="eyebrow">CATALOG</div><h2>模型目录</h2></div><span className="muted">{candidates.length} 个精确候选</span></div><div className="table-scroll"><table><thead><tr><th>精确模型</th><th>协议</th><th>类型</th><th>请求模式</th><th>生命周期</th><th>证据</th><th>调度原因</th><th>测活</th></tr></thead><tbody>{candidates.map(item => <tr key={item.directId}><td><strong>{item.directId}</strong><small>{item.channel} / {item.upstreamModel}</small></td><td>{protocolLabel(item.protocol)}</td><td>{kindLabel(item.kind)}</td><td>{streamingLabel(item.streaming)}</td><td><StateTag channel={item} /></td><td>{evidenceText(item.scheduling?.evidence)}</td><td>{reasonText(item.scheduling?.reasonCodes)}</td><td>{item.canaryEligible && item.kind === 'generation' ? <span className="state-tag success">可测活</span> : <span className="state-tag neutral">不适用</span>}</td></tr>)}</tbody></table></div></section></div>
}
...
function HealthTag({ value, lastStatusCode }: { value?: string; lastStatusCode?: number | null }) { const labels: Record<string, string> = { healthy: '健康', cooling: '冷却中', 'auth-failed': lastStatusCode === 403 ? '上游拒绝' : '认证失败', 'payment-blocked': '支付受限', degraded: '降级', untested: '未测试' }; const tone = value === 'healthy' ? 'success' : value === 'cooling' ? 'warning' : value ? 'danger' : 'neutral'; const diagnostic = value === 'auth-failed' && (lastStatusCode === 401 || lastStatusCode === 403) ? ` · HTTP ${lastStatusCode}` : ''; return <span className={`state-tag ${tone}`}>{labels[value ?? ''] ?? value ?? '未知'}{diagnostic}</span> }
...
function protocolLabel(value?: string) { return ({ 'openai-compatible': 'Chat Completions', responses: 'Responses', claude: 'Anthropic Messages' } as Record<string, string>)[value ?? ''] ?? value ?? '未知' }
...
function canaryErrorLabel(value?: string) { return ({ invalid_json: '响应不是有效 JSON', empty_body: '响应正文为空', missing_choices: '缺少 choices', content_null: '最终正文为 null', reasoning_only: '只有推理，没有最终正文', structured_content_unsupported: '没有可识别的文本正文', refusal_or_tool_call: '返回拒答或工具调用', short_content: '正文过短', empty_content: '没有最终正文', transport_error: '传输失败', authentication_failed: '认证失败', payment_blocked: '支付受限', rate_limited: '触发限流', upstream_server_error: '上游服务错误', protocol_or_path_error: '协议或路径错误', unexpected_status: '异常 HTTP 状态' } as Record<string, string>)[value ?? ''] ?? value ?? '未知失败' }
function formatCanaryDiagnostics(result: Json) { const values = [`HTTP ${result.statusCode ?? '—'}`, `正文 ${result.contentLength ?? 0} 字`, `响应 ${result.diagnostics?.bodyBytes ?? 0} B`]; if (result.diagnostics?.finishReason) values.push(`finish ${result.diagnostics.finishReason}`); if (result.diagnostics?.stopReason) values.push(`stop ${result.diagnostics.stopReason}`); if (result.diagnostics?.responseStatus) values.push(`status ${result.diagnostics.responseStatus}`); if (result.diagnostics?.reasoningPresent) values.push('检测到推理字段'); if (result.diagnostics?.usage?.outputTokens != null) values.push(`输出 ${result.diagnostics.usage.outputTokens} tokens`); return values.join(' · ') }
...
function operationLabel(value?: string) { return ({ 'startup-baseline': '启动基线', 'external-change': '外部变更', 'channel-create': '新增渠道', 'channel-import': '导入渠道', 'channel-update': '更新渠道', 'channel-retry': '重新尝试渠道', 'channel-delete': '删除渠道', 'model-update': '更新模型', 'model-protocol-update': '更新模型协议', 'channel-protocol-update': '更新渠道协议', 'protocol-update': '应用协议', 'alias-update': '更新别名', 'logical-model-create': '新增逻辑模型', 'logical-model-update': '更新逻辑模型', 'logical-model-delete': '删除逻辑模型', 'revision-prune': '整理 revision', 'model-sync': '同步模型', 'runtime-rollback': '运行回滚' } as Record<string, string>)[value ?? ''] ?? value ?? '未知' }
