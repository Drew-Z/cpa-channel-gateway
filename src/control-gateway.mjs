...
async function refresh(){try{const [status,models,usage,discovery]=await Promise.all([call('/admin/api/status'),call('/admin/api/models'),call('/admin/api/usage'),call('/admin/api/channel-discovery')]);restartRequired=status.restartRequired===true;$('summary').textContent='当前预约 '+status.reservations.length+' 个；24 小时请求 '+usage.summary.total+' 次'+(restartRequired?'；配置已更新，等待重启':'');$('channels').innerHTML='<h2>渠道</h2><table><tr><th>渠道</th><th>Base URL</th><th>状态</th><th>模型数</th><th>健康</th><th>操作</th></tr>'+status.channels.map(c=>'<tr><td>'+esc(c.name)+' ('+esc(c.id)+')</td><td><code>'+esc(c.baseUrl)+'</code></td><td class="'+(c.busy?'busy':'')+'">'+channelState(c)+(c.busy?' / busy':'')+'</td><td>'+c.modelCount+'</td><td>'+esc(c.health)+'</td><td>'+channelActions(c)+'</td></tr>').join('')+'</table>';$('discovery').innerHTML=discoveryHtml(discovery);$('usage').innerHTML=usageHtml(usage);populateModelSelect(models.data)}catch(e){$('summary').textContent=e.message}}
function populateModelSelect(groups){const select=$('model');const previous=select.value;select.replaceChildren(new Option('请选择要测试的精确模型',''));const candidates=groups.flatMap(group=>group.candidates||[]).filter((candidate,index,list)=>list.findIndex(item=>item.directId===candidate.directId)===index).sort((left,right)=>left.channel.localeCompare(right.channel)||left.upstreamModel.localeCompare(right.upstreamModel));const byChannel=new Map();for(const candidate of candidates){const optionGroup=byChannel.get(candidate.channel)||(()=>{const created=document.createElement('optgroup');created.label=candidate.channel;select.append(created);byChannel.set(candidate.channel,created);return created})();const states=[];if(candidate.staged)states.push('待测试');if(candidate.status==='stale')states.push('stale');if(candidate.busy)states.push('当前忙碌');if(!states.length)states.push(candidate.protocol);const option=new Option(candidate.upstreamModel+' ['+states.join('，')+']',candidate.directId);option.title=candidate.directId;if(candidate.busy)option.disabled=true;optionGroup.append(option)}if(candidates.some(candidate=>candidate.directId===previous&&!candidate.busy))select.value=previous;updateTestButton();$('testHint').textContent=restartRequired?'配置已更新，请重启应用后再执行模型测活':candidates.length?'请选择一个精确模型；当前忙碌渠道会暂时不可选；测试会发送固定诗词任务':'暂无可测模型，请先同步模型目录'}
function updateTestButton(){const selected=$('model').selectedOptions[0];$('testButton').disabled=restartRequired||!$('model').value||Boolean(selected?.disabled)}
...
function esc(v){return String(v).replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}
 $('loginButton').onclick=login;$('refreshButton').onclick=refresh;$('logoutButton').onclick=logout;$('syncButton').onclick=syncModels;$('model').onchange=updateTestButton;$('addChannelForm').onsubmit=e=>{e.preventDefault();addChannel()};$('addAndSyncButton').onclick=()=>addChannel(true);$('testButton').onclick=testModel;$('channels').onclick=e=>{const action=e.target.closest('.channelAction');const remove=e.target.closest('.deleteChannel');if(action)setChannelMode(action.dataset.id,action.dataset.mode);if(remove)deleteChannel(remove.dataset.id)};$('discovery').onclick=e=>{const importButton=e.target.closest('.importChannel');if(importButton)importChannel(importButton.dataset.id,importButton.dataset.sync==='true')};
</script></body></html>`
}
