...
async function refresh(){try{const [status,models,usage,discovery]=await Promise.all([call('/admin/api/status'),call('/admin/api/models'),call('/admin/api/usage'),call('/admin/api/channel-discovery')]);$('summary').textContent='当前预约 '+status.reservations.length+' 个；24 小时请求 '+usage.summary.total+' 次'+(status.restartRequired?'；配置已更新，等待重启':'');$('channels').innerHTML='<h2>渠道</h2><table><tr><th>渠道</th><th>Base URL</th><th>状态</th><th>模型数</th><th>健康</th><th>操作</th></tr>'+status.channels.map(c=>'<tr><td>'+esc(c.name)+' ('+esc(c.id)+')</td><td><code>'+esc(c.baseUrl)+'</code></td><td class="'+(c.busy?'busy':'')+'">'+channelState(c)+(c.busy?' / busy':'')+'</td><td>'+c.modelCount+'</td><td>'+esc(c.health)+'</td><td>'+channelActions(c)+'</td></tr>').join('')+'</table>';$('discovery').innerHTML=discoveryHtml(discovery);$('usage').innerHTML=usageHtml(usage);$('models').innerHTML='<table><tr><th>模型 ID</th><th>候选</th></tr>'+models.data.map(m=>'<tr><td>'+esc(m.id)+'</td><td>'+m.candidates.map(c=>esc(c.channel)+'/'+esc(c.upstreamModel)+' ['+esc(c.protocol)+(c.staged?', 待测试':'')+(c.status==='stale'?', stale 警告':'')+(c.busy?', busy':'')+']').join('<br>')+'</td></tr>').join('')+'</table>'}catch(e){$('summary').textContent=e.message}}
