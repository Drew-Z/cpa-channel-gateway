...
export function createControlGateway(config, {
  ...
  async function handleAdminRequest(request, response, url) {
    ...
    if (url.pathname === '/admin/api/connection' && request.method === 'GET') {
      sendJson(response, 200, adminConnection(request, url.searchParams.get('reveal') === '1'))
      return
    }
  ...
  function adminStatus() {
    ...
    const routing = configManager?.routing() ?? { stableAliases: config.stableAliases, pinnedAliases: config.pinnedAliases }
    return {
      ready: true,
...
<script nonce="${nonce}">
let csrf='';let restartRequired=false;let modelCandidates=new Map();
...
async function refresh(){try{const [status,models,usage,discovery,connection]=await Promise.all([call('/admin/api/status'),call('/admin/api/models'),call('/admin/api/usage'),call('/admin/api/channel-discovery'),call('/admin/api/connection')]);restartRequired=status.restartRequired===true;const revisionText=restartRequired?'；待重启 '+(status.pendingRevision||status.configRevision||'-')+'，运行中 '+(status.loadedRevision||'未知'):'；运行配置 '+(status.loadedRevision||status.configRevision||'未知');$('summary').textContent='当前预约 '+status.reservations.length+' 个；24 小时请求 '+usage.summary.total+' 次'+revisionText;renderConnection(connection);$('channels').innerHTML='<h2>渠道</h2><table><tr><th>渠道</th><th>Base URL</th><th>状态</th><th>模型数</th><th>健康</th><th>操作</th></tr>'+status.channels.map(c=>'<tr><td>'+esc(c.name)+' ('+esc(c.id)+')</td><td><code>'+esc(c.baseUrl)+'</code></td><td class="'+(c.busy?'busy':'')+'">'+channelState(c)+(c.busy?' / 当前忙碌':'')+'</td><td>'+c.modelCount+'</td><td>'+esc(healthLabel(c.health))+'</td><td>'+channelActions(c)+'</td></tr>').join('')+'</table>';$('discovery').innerHTML=discoveryHtml(discovery);$('usage').innerHTML=usageHtml(usage);$('routing').innerHTML=routingHtml(status.stableAliases||[]);$('models').innerHTML=modelsHtml(models.data);populateSyncChannelSelect(status.channels);populateModelSelect(models.data)}catch(e){$('summary').textContent=e.message}}
...
async function copyGatewayKey(){const button=$('copyGatewayKeyButton');button.disabled=true;try{const data=await call('/admin/api/connection?reveal=1');await copyValue(data.apiKey,'API key 已复制；页面仍保持掩码')}catch(e){$('connectionResult').className='error';$('connectionResult').textContent=e.message}finally{button.disabled=false}}
async function showGatewayKey(){const input=$('connectionApiKey');const button=$('showGatewayKeyButton');if(input.dataset.revealed==='true'){input.type='password';input.value=input.dataset.masked;input.dataset.revealed='false';button.textContent='显示';return}button.disabled=true;try{const data=await call('/admin/api/connection?reveal=1');input.type='text';input.value=data.apiKey;input.dataset.revealed='true';button.textContent='隐藏'}catch(e){$('connectionResult').className='error';$('connectionResult').textContent=e.message}finally{button.disabled=false}}
